 ...
 *
 * @param {Array} summary the parsed response body
 * @returns {Object} what is known about the slot, as { cutOffTime }
 */
function deriveOrderFacts(summary) {
  const nothing = { "cutOffTime": null };
  ...
  const slot = delivery["slot"];
  if (!isSet(slot)) return nothing;

  const cutOff = slot["cut_off_time"];

  return { "cutOffTime": typeof cutOff == 'string' && cutOff != "" ? cutOff : null };
}
