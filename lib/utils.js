  ...
  getCart() {
    return new Promise((resolve, reject) => {
      ...
      var req = https.request(options, (res) => {
        ...
        res.on('end', () => {
          if (res.statusCode == 401) {
            // the same error the order poll reacts to, so a token that went
          ...
          if (res.statusCode < 200 || res.statusCode >= 300) {
            const refused = new Error("Picnic refused to hand out the cart: " + describeBody(content));
            refused.statusCode = res.statusCode;
            return reject(refused);
          }
        ...
        reject(toError(e, "Retrieving the cart from " + options.hostname + " failed"));
      });

      req.on('timeout', () => {
        req.destroy(new Error("Picnic did not answer the cart request within " + options.timeout + "ms"));
      });
